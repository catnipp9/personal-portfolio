    ...
    // Further reduced default bottom margin for sections
    <div className={`mb-3 sm:mb-4 ${className}`}> 
        {/* Decreased font size for section titles */}
    ...
    const handleModalContentClick = (e) => {
        e.stopPropagation(); // Prevent modal close when clicking inside content
    };

    return (
        <div // Overlay
        // Significantly reduced opacity for the background overlay
        className="fixed inset-0 bg-black bg-opacity-10 flex items-center justify-center z-50 p-4 sm:p-6 md:p-8 transition-opacity duration-300 ease-in-out" 
        onClick={onClose}
        ...
        >
            {/* 1. Modal Header (Non-scrollable) - Further reduced top padding */}
            {/* Padding: pt-4 pb-3 sm:pt-5 sm:pb-4 px-5 sm:px-6 */}
            ...
            {/* 2. Content Area (flex-grow) - Reduced padding */}
            {/* Padding: p-4 sm:p-5 */}
            ...
            {/* Technologies Used Section - Overriding bottom margin */}
            <ModalSection 
                title="Technologies Used" 
                className="technologies-section !mb-1 sm:!mb-2" // Minimal bottom margin
            >
            ...
            {/* 3. Modal Footer (Links - Non-scrollable) - Reduced padding */}
            {/* Padding: p-4 sm:p-5 */}
